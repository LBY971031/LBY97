...
def stages(hw,k):
    ...
    return dict(pre=pre, mid_el=mid_el, mid_om=mid_om, mid=mid_el+mid_om,
                post_ds=post_ds, post_rv=post_rv, post=post_ds+post_rv,
                tot=pre+mid_el+mid_om+post_ds+post_rv)
...
md("阶段划分与参数",
   ["阶段","含义","本核算包含的科目","关键参数"],
   [["**前期**<br>投运前一次性","设备资本开支","服务器购置",
     "购置价 140~360 万元/台"],
    ["**中期**<br>五年运营期","随运行持续发生的支出","电费 + 运维（备件·人工·保修）",
...
for k in ["S1","S2"]:
    md(f"情景{k[-1]}　{PS[k]['nm']}　三阶段成本（元 / 百万Token）",
       ["平台 × 模型","**前期**<br>服务器购置","**中期**<br>电费+运维",
        "后期<br>处置","后期<br>残值","**后期小计**","**总计**"],
       [[NM[h],f"**{s['pre']:.3f}**",f"**{s['mid']:.3f}**",
         f"{s['post_ds']:.3f}",f"{s['post_rv']:.3f}",f"**{s['post']:.3f}**",f"**{s['tot']:.3f}**"]
...
for k in ["S1","S2"]:
    md(f"情景{k[-1]}　三阶段占比",
       ...
       [[NM[h],f"**{s['pre']/s['tot']*100:.1f}%**",f"{s['mid']/s['tot']*100:.1f}%",
         f"{s['post']/s['tot']*100:+.1f}%"]
        for h in HW for s in [stages(h,k)]])

md("五年生命周期绝对值（万元 / 台）",
   ["平台 × 模型","**前期**<br>购置","**中期 S1**<br>电费+运维","**中期 S2**<br>电费+运维",
    "后期<br>处置","后期<br>残值","**S1总计**","**S2总计**"],
   [[NM[h],f"{CAP[h]/1e4:.0f}",
     f"{(R[h]['kwh']*PS['S1']['price']+CAP[h]*MAINT*YEARS)/1e4:.1f}",
     f"{(R[h]['kwh']*PS['S2']['price']+CAP[h]*MAINT*YEARS)/1e4:.1f}",
     f"{CAP[h]*DISPOSAL/1e4:.1f}",f"−{CAP[h]*RESIDUAL/1e4:.0f}",
     f"**{(CAP[h]*(1+MAINT*YEARS+DISPOSAL-RESIDUAL)+R[h]['kwh']*PS['S1']['price'])/1e4:.1f}**",
     f"**{(CAP[h]*(1+MAINT*YEARS+DISPOSAL-RESIDUAL)+R[h]['kwh']*PS['S2']['price'])/1e4:.1f}**"]
    for h in HW])
